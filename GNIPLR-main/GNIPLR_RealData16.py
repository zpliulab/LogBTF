...
## time cost only CPU    https://blog.csdn.net/Caesar6666/article/details/113050925
import time
#time_start = time.clock()  # 记录开始时间  --  有问题 
time_start = time.time()  # 记录开始时间 CPU
...
pathname = "/home/lly/MATLAB/"  
Boolean = "Data"
## Mac
# pathname = "/Users/lilingyu/E/PhD/Matlab/"
# Boolean = "Rshell/Data"
## Mac
# os.chdir('/home/lly/R/Boolean/Data/DREAM3result_GNIPLR/RData/scData')
os.chdir(str(pathname)+'Boolean/GRISLI/SCODE-master/data16/')
# /Users/lilingyu/E/PhD/Matlab/Boolean/GRISLI/SCODE-master/data5/Label_data.txt
...
genenum = number_genes



ax_nl=0.04
n_d4 = str(dataname) + 'data.txt'
train_s=pd.read_csv(str(n_d4),sep='\t')
# train_s = train_s.transpose()
train_s.T.to_csv(str(dataname)+'_y1_10_d2_t.txt', sep='\t')



## ReadGround-truth GRN  
## Link file
fw=open('Interaction_A_List.txt')
...
## load new data 
file1 = open(str(dataname)+'_y1_10_d2_t.txt')
for p in file1:
	fla+=1
	t1 = p.split()
	n1=len(t1)
...
fw=open(str(dataname)+'e1_100_d2_p1.txt','w')
...
## Save results
relust1=pd.read_csv(str(dataname)+'e1_100_d2_p1.txt',sep='\t',names = range(4))
