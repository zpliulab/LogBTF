...
# dim(datahatA)    # 549 103
# dataoutput <- t(datahatA)
# colnames(dataoutput) <- colnames(X)
# rownames(dataoutput) <- rownames(X)
# View(datahatA[,1:10])
...
# write.csv(pred_lambda_min, "pred_lambda_min9.csv", row.names = F)
...
# write.csv(adj_matrix, "adj_matrix9dir.csv", row.names = F)
