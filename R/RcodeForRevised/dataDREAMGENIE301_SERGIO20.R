...
for (k in cellnum) {
  ...
  ## 2023.2.12  exprMatrix must contain the names of the genes as rownames.
  rownames(exprMatrix) <- seq(1,genenum,1)
  ...
  ## Error Error in randomForest.default(x, y, mtry = mtry, ntree = nb.trees, importance = FALSE,
...
## save
setwd(paste(pathway,"R/Boolean/", dataLinux,"/DREAM3result_GENIE301/",sep=""))
 if(method == 2){
