...
df <- readxl::read_excel("DREAM3scResult_BioRevised.xlsx", sheet="AUROC")
...
pAUROC <- ggplot(df1, aes(x=new_col, y=value))+
  ...
  geom_segment(x=1,xend=9,y=-0.15,yend=-0.15)+
  annotate("text",x=3.5, y=-0.1, label="Node 10", vjust=0.4)+
  geom_segment(x=10,xend=18,y=-0.15,yend=-0.15)+
...
# StAcc -------------------------------------------------------------------
df<-readxl::read_excel("DREAM3scResult_BioRevised.xlsx", sheet="StAcc")
x_level <- paste(df$Group1,df$Group2,sep="_")
df1 <- mydfdata(df)
...
pStAcc <- ggplot(df1, aes(x=new_col, y=value))+
  ...
  geom_segment(x=1,xend=8,y=-0.24,yend=-0.24)+
  geom_segment(x=9,xend=16,y=-0.24,yend=-0.24)+
  geom_segment(x=17,xend=24,y=-0.24,yend=-0.24)+
  coord_cartesian(clip = "off")
...
# AUPRsc10 -------------------------------------------------------------------
df<-readxl::read_excel("DREAM3scResult_BioRevised.xlsx", sheet="AUPRsc10")
x_level <- paste(df$Group1,df$Group2,sep="_")
df1 <- mydfdata(df)
...
# AUPRsc -------------------------------------------------------------------
df<-readxl::read_excel("DREAM3scResult_BioRevised.xlsx", sheet="AUPRsc50100")
x_level <- paste(df$Group1,df$Group2,sep="_")
df1 <- mydfdata(df)
...
df<-readxl::read_excel("DREAM3scResult_BioRevised.xlsx", sheet="Pre")[1:8,]
x_level <- paste(df$Group1,df$Group2,sep="_")
df1 <- mydfdata(df)
pPre10 <- ggplot(df1, aes(x=new_col, y=value))+
  ...
  geom_segment(x=1,xend=8,y=-0.16,yend=-0.16)+
  coord_cartesian(clip = "off")
...
# Pre 50---------------------------------------------------------------------
df<-readxl::read_excel("DREAM3scResult_BioRevised.xlsx", sheet="Pre")[9:24,]
x_level <- paste(df$Group1,df$Group2,sep="_")
df1 <- mydfdata(df)
...
pPre50 <- ggplot(df1, aes(x=new_col, y=value))+
  ...
  geom_segment(x=1,xend=8,y=-0.046,yend=-0.046)+
  geom_segment(x=9,xend=16,y=-0.046,yend=-0.046)+
  coord_cartesian(clip = "off")
...
pAUROC1 <- ggplot(df1, aes(x=new_col, y=value))+
  ...
  scale_x_discrete(labels = str_split_fixed(x_level, "_", 2)[,1],
                   guide = "prism_offset")+
  scale_y_continuous(limits = c(0.30, 0.7),
                     breaks = seq(0.30, 0.7, by=0.05))+
  theme_prism(axis_text_angle =45,
              base_line_size = 0.1,
              base_fontface = "plain",
              # base_family = "serif",
...
pAUPR1 <- ggplot(df1, aes(x=new_col, y=value))+
  ...
  geom_segment(x=1,xend=2,y=-0.28, yend=-0.28)+
  geom_segment(x=3,xend=4,y=-0.28, yend=-0.28)+
  geom_segment(x=5,xend=6,y=-0.28, yend=-0.28)+
  coord_cartesian(clip = "off")
...
pLogBTF <- ggplot(df1, aes(x=new_col, y=value))+
  stat_boxplot(geom = "errorbar",width=0.2)+
  geom_boxplot(outlier.shape = 1, aes(fill=group), show.legend = F)+
  scale_fill_manual(values = c("#d4d66f","#8ea8d0"))+
  scale_x_discrete(labels = str_split_fixed(x_level, "_", 2)[,1],
                   guide = "prism_offset") +
  ...
  geom_segment(x=1,xend=2,y=0.780, yend=0.780)+
  geom_segment(x=3,xend=4,y=0.780, yend=0.780)+
  geom_segment(x=5,xend=6,y=0.780, yend=0.780)+
  coord_cartesian(clip = "off")
...
# 2023 New Revised -- AUC + DyAcc, AUROC, AUPR , SIGN = 1 ----------------------------
# pdf("FigurescBioRevised/sc2Method.pdf",width = 8, height = 4)
cowplot::plot_grid(plotlist = list(LogBTF[[11]], LogBTF[[7]], LogBTF[[10]] ), 
                   nrow = 1, rel_widths = c(1.5, 1.5, 1.5),
                   labels = c('(a)','(b)','(c)'),
                   scale = c(0.95)) 
# dev.off()
...
# # AUROC, AUPR, PRE ------------------------------------------------------
## save 18*6
# pdf("FigurescBioRevised/sc8Method.pdf",width = 20, height = 6.5)
cowplot::plot_grid(plotlist = list(LogBTF[[1]], LogBTF[[3]], LogBTF[[4]], LogBTF[[5]], LogBTF[[6]]), 
                   nrow = 1, rel_widths = c(2.5, 0.6, 0.95, 0.95, 1.65),
                   labels = c('(a)','(b)','','(c)'),
                   scale = c(0.98))
# dev.off()
